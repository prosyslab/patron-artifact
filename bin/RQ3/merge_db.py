...
local_config = {"IS_SCRATCH": False, "DB_PATHS": [], "DONOR_PATHS": dict()}
...
def mk_exp_dbs():
    ...
    second_out, second_err = run_process(
        ['python3',
         os.path.join(FILE_PATH, 'patron.py'), '-db', '-donorpath', 'data/CWE-patches'])
    if second_err:
    ...
    third_out, third_err = run_process(
        ['python3',
         os.path.join(FILE_PATH, 'patron.py'), '-db', '-donorpath', 'data/CVE-patches'])
    if third_err:
...
def run():
    for path in local_config['DB_PATHS']:
        ...
        for cont in os.listdir(path):
            ...
            local_config['DONOR_PATHS'][cont] = local_config['DONOR_PATHS'][cont] + [
                os.path.join(cont_path, f) for f in os.listdir(cont_path)
            ]
