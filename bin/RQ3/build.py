...
#         packages = [os.path.join(config.configuration['LIST_DIR'], 'test.txt')]
#     else:
...
def kill_processes() -> bool:
    log(ALL, "Building process Timed out! Assuming there is a lock on apt process. Killing the apt process ...")
    try:
...
def run_smake(packages:list) -> tuple:
    ...
    tsvfile = open(os.path.join(BUILD_LOG_PATH, 'smake_stat_{}.tsv'.format(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))), 'a')
...
# def run_smake_pipe(package:str) -> None:
#     smake_out_dir = os.path.join(config.configuration['PKG_DIR'], 'smake_out')
#     if not os.path.exists(smake_out_dir):
#         os.mkdir(smake_out_dir)
#     BUILD_LOG_PATH = os.path.join(config.configuration['OUT_DIR'], "build_infos")
#     if not os.path.exists(BUILD_LOG_PATH):
#         os.mkdir(BUILD_LOG_PATH)
#     tsvfile = open(os.path.join(BUILD_LOG_PATH, 'build_stat_{}.tsv'.format(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))), 'a')
#     writer = csv.writer(tsvfile, delimiter='\t')
#     writer.writerow(['Package', 'Build Time', 'Error Msg'])
#     tsvfile.flush()
#     package = package.strip()
#     if not os.path.exists(os.path.join(smake_out_dir, package)):
#         os.mkdir(os.path.join(smake_out_dir, package))
#     os.chdir(config.configuration['PKG_DIR'])
#     log(ALL, f"Building {package} ...")
#     smake(package, tsvfile, writer, smake_out_dir, 0)
#     tsvfile.close()
#     log(INFO, "Building is done.")
...
def smake(package: str, tsvfile: TextIO, writer: csv.writer, smake_out_dir: str) -> bool:
    ...
    try:
        proc = subprocess.Popen([os.path.join(config.configuration['PKG_DIR'], 'build-deb.sh'), package],
...
def print_build_summary(results:tuple) -> None:
    ...
    for i, success in enumerate(success_list):
        if success:
            log(ALL, f"smake for {pkg_path_list[i].split('/')[-1]} was successfully done and saved at {pkg_path_list[i]}.")
        else:
