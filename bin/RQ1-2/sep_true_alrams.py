...
def parse_args():
    ...
    parser.add_argument("--skip-intro, -s",
                        action="store_true",
                        default=True,
    ...
    logger_wrapper(0, "logs are saved in " + config.configuration["OUT_DIR"] + "/ALARM_SEP_log.txt")
    logger_wrapper(0, "task objectives: separate true alarms")
    logger_wrapper(0, "target benchmark_set: " + configuration["BENCHMARK_SET"])
...
def safty_check_patron(dir, alarm_paths):
    ...
    if not os.path.exists(dir + "/bug/sparrow-out"):
        logger_wrapper(1, alram_num + "/sparrow-out does not exist. Skip this project.")
        failed.append(alram_num)
        return False
    if os.path.exists(dir + "/bug/sparrow-out/taint/datalog_full"):
        logger_wrapper(1, alram_num + "/datalog_full file already exists. Skip this project.")
        failed.append(alram_num)
        return False
    if not os.path.exists(dir + "/bug/sparrow-out/taint/datalog"):
        logger_wrapper(1, alram_num + "/datalog file does not exist. Skip this project.")
        failed.append(alram_num)
        return False
    for path in alarm_paths:
        if not os.path.exists(path):
            logger_wrapper(
                1,
                alram_num + " " + path + "th alarm do not exist in the datalog. Skip this project.")
            failed.append(alram_num)
            return False
    if alarm_paths == []:
        logger_wrapper(1, alram_num + ",no alarm in the project. Skip this project.")
        failed.append(alram_num)
        return False
    return True
    ...
    for dir in os.listdir(PATRON_BENCH_PATH):
        if dir in configuration["ID"]:
            ...
            with open(os.path.join(full_dir, "label.json"), "r") as f:
                ...
                # safety check
                try:
                    alarms = label["TRUE-ALARM"]["ALARM-DIR"] + label["OTHER-ALARMS"]["ALARM-DIR"]
                except KeyError:
                    logger_wrapper(
                        1, dir +
                    ...
                    continue
                alarm_paths = [(full_dir + "/bug/sparrow-out/taint/datalog/" + alarm)
                               for alarm in alarms]
                if not safty_check_patron(full_dir, alarm_paths):
                    continue
                # critical area
                os.system("mv " + full_dir + "/bug/sparrow-out/taint/datalog " + full_dir +
                          "/bug/sparrow-out/taint/datalog_full")
                os.system("mkdir " + full_dir + "/bug/sparrow-out/taint/datalog")
                for alarm in alarms:
                    os.system("cp -r " + full_dir + "/bug/sparrow-out/taint/datalog_full/" + alarm +
                              " " + full_dir + "/bug/sparrow-out/taint/datalog/")
...
def safty_check_patchweave(dir, donor_alarm_paths, donee_alarm_paths):
    ...
    if not os.path.exists(dir + "/donor/bug/sparrow-out"):
        logger_wrapper(1, alarm_num + "/donor/bug/sparrow-out does not exist. Skip this project.")
        failed.append(alarm_num)
        return False
    if not os.path.exists(dir + "/donee/sparrow-out"):
        logger_wrapper(1, alarm_num + "/donee/sparrow-out does not exist. Skip this project.")
        failed.append(alarm_num)
        return False
    if not os.path.exists(dir + "/donor/bug/sparrow-out/taint/datalog"):
        logger_wrapper(
            1,
            alarm_num + "/donor/bug/sparrow-out/taint/datalog does not exist. Skip this project.")
        failed.append(alarm_num)
        return False
    ...
    if not os.path.exists(dir + "/donee/sparrow-out/taint/datalog"):
        logger_wrapper(
            1, alarm_num + "/donee/sparrow-out/taint/datalog does not exist. Skip this project.")
        failed.append(dir)
        return False
    ...
    for path in donor_alarm_paths:
        if not os.path.exists(path):
            logger_wrapper(1,
                           path + "th alarm do not exist in the donor datalog. Skip this project.")
            failed.append(alarm_num)
            return False
    ...
    if donor_alarm_paths == []:
        logger_wrapper(1, alarm_num + ", no alarm in the donor project. Skip this project.")
        failed.append(alarm_num)
        return False
    if donee_alarm_paths == []:
        logger_wrapper(1, alarm_num + ", no alarm in the donee project. Skip this project.")
        failed.append(alarm_num)
        return False
    return True
        ...
        if dir in configuration["ID"]:
            ...
            with open(os.path.join(full_dir, "label.json"), "r") as f:
                ...
                try:
                    ...
                    logger_wrapper(
                        1, dir + ", no donor or donee alarms in the label file. Skip this project.")
                    continue
                donor_alarm_paths = [(full_dir + "/donor/bug/sparrow-out/taint/datalog/" + alarm)
                                     for alarm in donor_alarms]
                donee_alarm_paths = [(full_dir + "/donee/sparrow-out/taint/datalog/" + alarm)
                                     for alarm in donee_alarms]
                if not safty_check_patchweave(full_dir, donor_alarm_paths, donee_alarm_paths):
                    continue
                os.system("mv " + full_dir + "/donor/bug/sparrow-out/taint/datalog " + full_dir +
                          "/donor/bug/sparrow-out/taint/datalog_full")
                os.system("mkdir " + full_dir + "/donor/bug/sparrow-out/taint/datalog")
                for alarm in donor_alarms:
                    os.system("cp -r " + full_dir + "/donor/bug/sparrow-out/taint/datalog_full/" +
                              alarm + " " + full_dir + "/donor/bug/sparrow-out/taint/datalog/")
                os.system("mv " + full_dir + "/donee/sparrow-out/taint/datalog " + full_dir +
                          "/donee/sparrow-out/taint/datalog_full")
                os.system("mkdir " + full_dir + "/donee/sparrow-out/taint/datalog")
                for alarm in donee_alarms:
                    os.system("cp -r " + full_dir + "/donee/sparrow-out/taint/datalog_full/" +
                              alarm + " " + full_dir + "/donee/sparrow-out/taint/datalog/")
                logger_wrapper(0, "Done separating " + full_dir)
                successed.append(dir)
    logger_wrapper(0, "Separate true alarms done.")
